...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        // File handling
        $upload_dir = __DIR__ . DIRECTORY_SEPARATOR . trim(str_replace("/", DIRECTORY_SEPARATOR, $filepath), DIRECTORY_SEPARATOR); // Ensure this directory exists
        ...
        if (move_uploaded_file($file_tmp_path, $file_dest_path)) {
            ...
            $dt = new DateTime($created_at);$dt->setTimezone(new DateTimeZone("Asia/Manila"));
            $created_at_mysql = $dt->format("Y-m-d H:i:s");

            $stmt = conn()->prepare("INSERT INTO inference_model (algo, size, filename, file_extension, filepath, fullpath, accuracy, created_at) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)");

            $stmt->bind_param("sissssds", $algo, $size, $filename, $file_extension, $filepath, $fullfilepath, $accuracy, $created_at_mysql);
