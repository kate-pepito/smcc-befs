...
from befs.route.responses import FileModelData, FileModelResponse, MLModelMetadata, SessionValidateRequest, SessionValidateResponse, TrainCreateSessionPost, TrainCreateSessionRequest, TrainCreateSessionResponse, TrainDestroySessionResponse, TrainSessionsGet, TrainingStatesResponse
...
async def get_train_session(data: TrainCreateSessionRequest) -> TrainCreateSessionResponse:
    return http_get(f"{settings.MAIN_BASE_URL}/api/get_train_session", data.model_dump(exclude={'algo'}))

async def create_train_session_api(data: TrainCreateSessionPost):
    return http_get(f"{settings.MAIN_BASE_URL}/api/create_train_session", data.model_dump())

async def validate_train_session(data: SessionValidateRequest) -> SessionValidateResponse:
    return http_get(f"{settings.MAIN_BASE_URL}/api/validate_session", data.model_dump())

async def invalidate_train_session(data: SessionValidateRequest) -> TrainDestroySessionResponse:
    return http_post(f"{settings.MAIN_BASE_URL}/api/invalidate_session", data.model_dump())

async def invalidate_train_session_token(data: SessionValidateRequest) -> TrainDestroySessionResponse:
    return http_post(f"{settings.MAIN_BASE_URL}/api/invalidate_session_token", data.model_dump())

async def get_train_sessions() -> TrainSessionsGet:
    return http_get(f"{settings.MAIN_BASE_URL}/api/get_all_sessions")
...
async def update_training_state(token: str, state: TrainingStatesResponse):
    await http_post(f"{settings.MAIN_BASE_URL}/api/train/update?token={token}", data=state.model_dump())